...
def _ensure_geometry(data, crs=None):
    ...
    """
    pass
crs_mismatch_error = "CRS mismatch between CRS of the passed geometries and 'crs'. Use 'GeoDataFrame.set_crs(crs, allow_override=True)' to overwrite CRS or 'GeoDataFrame.to_crs(crs)' to reproject geometries. "
...
class GeoDataFrame(GeoPandasBase, DataFrame):
    ...
    def __setstate__(self, state):
        ...
        if isinstance(state, dict):
            ...
            if crs is not None and (not HAS_PYPROJ):
                raise ImportError("Unpickling a GeoDataFrame with CRS requires the 'pyproj' package, but it is not installed or does not import correctly. ")
    ...
    def to_arrow(self, *, index=None, geometry_encoding='WKB', interleaved=True, include_z=None):
    ...
    def to_parquet(self, path, index=None, compression='snappy', geometry_encoding='WKB', write_covering_bbox=False, schema_version=None, **kwargs):
    ...
    def dissolve(self, by=None, aggfunc='first', as_index=True, level=None, sort=True, observed=False, dropna=True, method='unary', **kwargs):
        """
